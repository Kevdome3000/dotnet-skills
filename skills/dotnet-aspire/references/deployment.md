...
- [Preview publish and manifest flows](#preview-publish-and-manifest-flows)
...
4. Preview publish or manifest-oriented flows only when you explicitly need artifact generation or another deployment system
...
## Preview publish and manifest flows

The Aspire CLI currently exposes a preview publish command:

- `aspire publish`

Use it when the task explicitly calls for:

- deployment artifact generation
- manifest-oriented handoff to another toolchain
- inspection of the publish output instead of a direct `azd` deployment

Treat this flow as version-sensitive and preview-sensitive. Verify current docs and command behavior before encoding it into long-lived repo automation.
...
5. The team can explain why the target is ACA, App Service, or a preview publish flow.
